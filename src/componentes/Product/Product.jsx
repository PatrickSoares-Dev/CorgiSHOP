...
const Product = () => {
  ...
  const PrevArrow = (props) => {
    ...
    return (
      <button className={`${className} absolute top-0 left-0 z-10 bg-black text-black rounded-full p-2 shadow-md`} onClick={onClick}>
        PREV
      </button>
    );
  };
  ...
  const NextArrow = (props) => {
    const { className, onClick } = props;
    return (
      <button className={`${className} absolute top-0 right-0 z-10 bg-black text-black rounded-full p-2 shadow-md`} onClick={onClick}>
        NEXT
      </button>
    );
  };
  ...
  return (
    <div className="w-full max-w-screen-2xl mx-auto">
      <div className="max-w-lg text-center sm:text-left ml-4 border-s-4 border-sky-400">
        <h2 className="ml-4 text-2xl text-black sm:text-2xl md:text-4xl mb-2">
          Novos produtos
        </h2>
      </div>
      <Slider {...settings}>
        {products.map((product, index) => (
          ...
          >
            ...
            <div className="mt-4 px-5 pb-5">
              ...
              <a
                href="#"
                className="flex items-center justify-center rounded-md bg-sky-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-blue-300"
              >
                <svg
