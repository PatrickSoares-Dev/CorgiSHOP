...
const ViewProducts = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`http://18.212.4.156:8080/product?id=${productId}`);
        ...
        <div class="container mx-auto px-4">
            ...
            <ol role="list" class="flex items-center">
                ...
                <span class="mx-2 text-gray-400">/</span>
                    <div class="-m-1">
                        <a href="#" class="rounded-md p-1 text-sm font-medium text-gray-600 focus:text-gray-900 focus:shadow hover:text-gray-800">{productJSON.Categoria}</a>
                    </div>
                </div>
                </li>
          ...
          <div className="lg:col-span-3 lg:row-end-1">
            <div className="lg:flex lg:items-start">
             <div className="lg:order-2 lg:ml-5">
                <div className="max-w-xl overflow-hidden rounded-lg transform transition-transform hover:scale-105 border border-gray-300">
                    <img className="h-full w-full max-w-full object-cover shadow-md" src={mainImage} alt="" />
                </div>
             </div>

              <div className="mt-2 w-full lg:order-1 lg:w-32 lg:flex-shrink-0">
                <div className="flex flex-row items-start lg:flex-col">
                  <button type="button" className="flex-0 aspect-square mb-3 h-20 overflow-hidden rounded-lg border border-gray-300 text-center" nClick={() => handleImageClick(product.imagem2)}>
                    <img className="h-full w-full max-w-full object-cover shadow-md" src={product.imagem2} alt="" />
                  </button>
                    <button type="button" class="flex-0 aspect-square mb-3 h-20 overflow-hidden rounded-lg border border-gray-300 text-center" nClick={() => handleImageClick(product.imagem3)}>
                        <img class="h-full w-full max-w-full object-cover shadow-md" src={product.imagem3} alt="" />
                    </button>
                    <button type="button" class="flex-0 aspect-square mb-3 h-20 overflow-hidden rounded-lg border border-gray-300 text-center" nClick={() => handleImageClick(product.imagem4)}>
                        <img class="h-full w-full max-w-full object-cover shadow-md" src={product.imagem4} alt="" />
                    </button>
                    </div>
                </div>
                </div>
            ...
            <div class="lg:col-span-2 lg:row-span-2 lg:row-end-2 " >
                <h1 class="sm: text-2xl font-bold text-sky-500 sm:text-3xl">{product.nome}</h1>
                ...
                <p class="ml-2 text-sm font-medium text-gray-500">{productJSON.Avaliacao} Avaliações</p>
                </div>
                ...
                <label class="">
                    ...
                    <p class="peer-checked:bg-blue-500 peer-checked:text-white rounded-lg border border-grey-400 px-6 py-2 font-bold cursor-pointer">Tam. 00</p>
                </label>
                <label class="">
                    <input type="radio" name="type" value="Whole Bean" class="peer sr-only" />
                    <p class="peer-checked:bg-blue-500 peer-checked:text-white rounded-lg border border-grey-400 px-6 py-2 font-bold cursor-pointer">Tam. 01</p>
                </label>
                <label class="">
                    <input type="radio" name="type" value="Groud" class="peer sr-only" />
                    <p class="peer-checked:bg-blue-500 peer-checked:text-white rounded-lg border border-grey-400 px-6 py-2 font-bold cursor-pointer">Tam. 02</p>
                </label>
                </div>
                ...
                <div class="mt-3 flex select-none flex-wrap items-center gap-1">
                <label class="border border-gray-300 rounded-lg px-6 py-2 cursor-pointer">
                ...
                <h1 className="text-3xl font-bold pr-2">{"R$ " + product.valorProduto}</h1>
            ...
            <div class="lg:col-span-5">
                ...
                <nav class="flex gap-4">
                    ...
                    Avaliações
                    <span class="ml-2 block rounded-full bg-blue-500 px-2 py-px text-xs font-bold text-gray-100">{productJSON.Avaliacao}</span>
                    </a>
                </nav>
                </div>
